 ...
 */
contract SecondaryNFTMarketPlace is AccessControlEnumerable, IERC721Receiver, ReentrancyGuard, IERC1155Receiver, EIP712 {
  ...
  event NftListed(uint256 indexed tokenId, address indexed nftAddress, address indexed seller, uint256 price, address erc20Token, uint256 quantity);
  ...
  mapping(address => mapping(uint256 => mapping(address => Sale))) private mapSale;
  ...
  */
  bytes32 public constant FIXED_PRICE_TYPEHASH =
    keccak256('FixedPriceSecondary(uint256 tokenId,uint256 price,uint256 quantity,address erc20Token,address seller,address nftAddress)');
  bytes32 public constant AUCTION_TYPEHASH =
    keccak256(
      'AuctionSecondary(uint256 tokenId,uint256 basePrice,uint256 salePrice,uint256 quantity,address erc20Token,address seller,address nftAddress)'
    );
   ...
   */
  constructor(Fee memory _platformFee, address _rootAdmin) EIP712('SecondaryNFTMarketPlace', '0.0.1') {
  ...
  function _listNft(
    ...
    require(_nftAddress != address(0), 'Sell Nfts: Nft Address Can Not Be Zero');
    require(_isNFT(_nftAddress), 'Sell Nfts: Not confirming to an NFT contract');

    _setSaleDetails(_tokenId, _price, _quantity, _erc20Token, _seller, _nftAddress);

    emit NftListed(_tokenId, _nftAddress, _seller, _price, _erc20Token, _quantity);
  }
  ...
  function _setSaleDetails(
    ...
    Sale storage NftForSale = mapSale[_nftAddress][_tokenId][_sellerAddress];
  ...
  function buyNft(
    ...
    _verifySignature(_tokenId, _sellerAddress, _nftAddress, _saleType, _signature);

    if (_saleType == TYPE_SALE) {
      require(_quantity > 0, 'BuyNfts: Zero Quantitiy');
      _NftSaleFixedPrice(_tokenId, _quantity, _nftAddress, _sellerAddress);
  ...
  function fiatPurchase(
    ...
    if (_saleType == TYPE_SALE) {
      require(_quantity > 0, 'BuyNfts: Zero Quantitiy');
      _NftSaleFixedPriceFiat(_tokenId, _quantity, _nftAddress, _sellerAddress, _buyer);
    } else if (_saleType == TYPE_AUCTION) {
      _NftAuctionInstantBuyFiat(_tokenId, _nftAddress, _sellerAddress, _buyer);
    } else {
      revert('Sale type is invalid');
    }
  }
  ...
  function cancelListing(
    ...
    Sale storage NftForSale = mapSale[_nftAddress][_tokenId][_sellerAddress];
    require(msg.sender == NftForSale.seller || _isAdmin(), 'Cancel Listing : Only Seller or admin can cancel the Listing');
    require(NftForSale.quantity > 0, 'Buy Nfts :No active sale');
    _verifySignature(_tokenId, _sellerAddress, _nftAddress, TYPE_SALE, _signature);

    ///for erc721
    if (IERC721(_nftAddress).supportsInterface(ERC721InterfaceId)) {
      IERC721(_nftAddress).transferFrom(address(this), NftForSale.seller, _tokenId);
    }
    ///for erc1155
    else if (IERC1155(_nftAddress).supportsInterface(ERC1155InterfaceId)) {
      IERC1155(_nftAddress).safeTransferFrom(address(this), NftForSale.seller, _tokenId, NftForSale.quantity, '');
    }
   ...
   * @param _nftAddress NFT contract address
   */
  function _createAuction(Auction memory auctionDetails, address _nftAddress) internal {
    require(auctionDetails.basePrice > 0, 'CreateAuction : Not Accept Zero BasePrice');
    require(auctionDetails.quantity != 0, 'CreateAuction: Zero Quantity');
    require(auctionDetails.auctioner != address(0), 'CreateAuction: Zero Address');
    require(_nftAddress != address(0), 'CreateAuction: Zero Address');
    require(_isNFT(_nftAddress), 'CreateAuction: Not confirming to an NFT contract');
  ...
  function placeBid(
    ...
    require(NftOnAuction.quantity > 0, 'Place Bid: Bid for non existing auction');
    ...
    require(_price > NftOnAuction.bidAmount, 'The price is less then the previous bid amount');
    _verifySignature(_tokenId, _auctioner, _nftAddress, TYPE_AUCTION, _signature);

    if (NftOnAuction.erc20Token == address(0)) {
      require(msg.value == _price, 'Amount received and price should be same');
      require(msg.value > NftOnAuction.bidAmount, 'Amount received should be grather than the current bid');
  ...
  function settleAuction(
    ...
    require(msg.sender == NftOnAuction.auctioner || _isAdmin(), 'Settle Auction : Only Seller or admin can settle the auction');
    _verifySignature(_tokenId, _auctioner, _nftAddress, TYPE_AUCTION, _signature);

    if (NftOnAuction.currentBidder != address(0)) {
      if (IERC721(_nftAddress).supportsInterface(ERC721InterfaceId)) {
        IERC721(_nftAddress).transferFrom(address(this), NftOnAuction.currentBidder, _tokenId);
      } else if (IERC1155(_nftAddress).supportsInterface(ERC1155InterfaceId)) {
        IERC1155(_nftAddress).safeTransferFrom(address(this), NftOnAuction.currentBidder, _tokenId, NftOnAuction.quantity, '');
      }
      ...
      if (IERC721(_nftAddress).supportsInterface(ERC721InterfaceId)) {
        IERC721(_nftAddress).transferFrom(address(this), NftOnAuction.auctioner, _tokenId);
      } else if (IERC1155(_nftAddress).supportsInterface(ERC1155InterfaceId)) {
        IERC1155(_nftAddress).safeTransferFrom(address(this), NftOnAuction.auctioner, _tokenId, NftOnAuction.quantity, '');
      }
    }
  ...
  function cancelAuction(
    ...
    Auction storage NftOnAuction = mapAuction[_nftAddress][_tokenId][_auctioner];
    require(_isAdmin() || msg.sender == NftOnAuction.auctioner, 'Cancel Auction: Admin or Auctioner can cancel auction');
    require(NftOnAuction.tokenId == _tokenId, 'Cancel Auction: Token id is not on auction');
    require(NftOnAuction.quantity > 0, 'Cancel Auction: Token id is not on auction');
    _verifySignature(_tokenId, _auctioner, _nftAddress, TYPE_AUCTION, _signature);
   ...
   * @param to token reciver address
  ...
  function onERC721Received(
    ...
    (uint256 price, uint256 basePrice, address erc20Token, uint256 saleType) = abi.decode(data, (uint256, uint256, address, uint256));
    ...
    if (saleType == 1) {
      ...
      _createAuction(Auction(tokenId, basePrice, price, erc20Token, 1, from, address(0), 0), nftAddress);
    } else {
      require(saleType == TYPE_SALE || saleType == TYPE_AUCTION, 'Invalid Type');
    }
  ...
  function onERC1155Received(
    ...
    if (saleType == 1) {
      ...
      _createAuction(Auction(id, basePrice, price, erc20Token, value, from, address(0), 0), nftAddress);
    } else {
      revert('Sale Type Is Invalid');
    }
  ...
  function _fundTransfer(
    ...
    if (_tokenErc20 == address(0)) {
      if (_royaltyReceiver != address(0) && royaltyAmount > 0) {
        (bool isRoyaltyTransferSuccess, ) = payable(_royaltyReceiver).call{ value: royaltyAmount }('');
        require(isRoyaltyTransferSuccess, 'Fund Transfer: Transfer to royalty receiver failed.');
      }
      ...
      if (_isDirectPurchase) {
        if (_royaltyReceiver != address(0) && royaltyAmount > 0) {
          IERC20(_tokenErc20).safeTransferFrom(msg.sender, _royaltyReceiver, royaltyAmount);
        }
        if (platformFee.receiver != address(0) && platformProfit > 0) {
        ...
        if (_royaltyReceiver != address(0) && royaltyAmount > 0) {
          IERC20(_tokenErc20).safeTransfer(_royaltyReceiver, royaltyAmount);
        }
        if (platformFee.receiver != address(0) && platformProfit > 0) {
    ...
    emit FundTransfer(Fee(_royaltyReceiver, royaltyAmount), Fee(_seller, sellerProfit), Fee(platformFee.receiver, platformProfit));
  }
  ...
  function _getRoyaltyInfo(
    ...
    if (IERC2981(_contract).supportsInterface(royaltyInterfaceId)) {
      (user, royaltyAmount) = IERC2981(_contract).royaltyInfo(_tokenId, _amount);
    }
  }
  ...
  function _payOut(
    ...
    _fundTransfer(_totalAmount, royaltyValue, _currency, _isDirectPurchase, _seller, royaltyReceiver);
  }
  ...
  function _NftSaleFixedPrice(
    ...
    require(NftForSale.quantity > 0, 'Buy Nfts:No NFT is availabe for purchase');
    require(msg.sender != NftForSale.seller, 'Buy Nfts: Owner Is Not Allowed To Buy Nfts');
    require(_quantity <= NftForSale.quantity, 'Buy Nfts: Exceeds max quantity');
    ...
    if (NftForSale.erc20Token == address(0)) {
      ...
      require(IERC20(NftForSale.erc20Token).allowance(buyer, address(this)) >= buyAmount, 'Less allowance');

      IERC20(NftForSale.erc20Token).safeTransferFrom(buyer, address(this), buyAmount);
    }
    _NftSale(_tokenId, _quantity, _nftAddress, buyAmount, buyer, NftForSale);
  }
  ...
  function _NftAuctionInstantBuy(
    ...
    require(NftOnAuction.quantity > 0, 'Auction Instant Buy: Buy for non existing auction');
    require(!_isAdmin(), 'Auction Instant Buy: Admin not allowed to perform purchase');

    require(msg.sender != NftOnAuction.auctioner, 'Auction Instant Buy : Sellernot allowed to perform purchase');
    ...
    if (NftOnAuction.erc20Token == address(0)) {
      ...
      require(checkAllowance >= NftOnAuction.salePrice, 'Place Bid : Allowance is Less then Price');
  ...
  function _NftAuctionInstantBuyFiat(
    ...
    require(_buyer != NftOnAuction.auctioner, 'Auction Instant Buy : Sellernot allowed to perform purchase');
    require(NftOnAuction.salePrice > NftOnAuction.bidAmount, 'Auction Instant Buy: bid exceeds sale price');
  ...
  function _NftSale(
    ...
    if (IERC721(_nftAddress).supportsInterface(ERC721InterfaceId)) {
      require(_quantity == 1, 'BuyNfts: ERC721Token accept only one token quantity');
      //nft transfer
      IERC721(_nftAddress).transferFrom(address(this), _buyer, _tokenId);
    } else if (IERC1155(_nftAddress).supportsInterface(ERC1155InterfaceId)) {
      //transfer nfts
      IERC1155(_nftAddress).safeTransferFrom(address(this), _buyer, _tokenId, _quantity, '');
    }
    _payOut(_nftAddress, _buyAmount, NftForSale.erc20Token, NftForSale.seller, true);

    Sale storage nftOnSaleStorage = mapSale[_nftAddress][_tokenId][NftForSale.seller];
    nftOnSaleStorage.quantity = nftOnSaleStorage.quantity - _quantity;

    if (NftForSale.quantity == 0) {
      delete mapSale[_nftAddress][_tokenId][NftForSale.seller];
    }
    emit NftSold(_tokenId, _nftAddress, NftForSale.seller, NftForSale.price, NftForSale.erc20Token, _buyer, _quantity);
  }
  ...
  function _NftAuction(
    ...
    if (IERC721(_nftAddress).supportsInterface(ERC721InterfaceId)) {
      ...
      IERC1155(_nftAddress).safeTransferFrom(address(this), msg.sender, _tokenId, NftOnAuction.quantity, '');
    }

    _payOut(_nftAddress, NftOnAuction.salePrice, NftOnAuction.erc20Token, NftOnAuction.auctioner, true);
  ...
  function _hashFixedPriceTypedData(
    ...
    return
      _hashTypedDataV4(
        keccak256(
          abi.encode(FIXED_PRICE_TYPEHASH, tokenId, NftForSale.price, NftForSale.quantity, NftForSale.erc20Token, NftForSale.seller, nftAddress)
        )
      );
  }
  ...
  function _verifySignature(
    ...
    address signer;
    if (_saleType == TYPE_SALE) {
      signer = _getSigner(_hashFixedPriceTypedData(_tokenId, _auctioner, _nftAddress), _signature);
    } else if (_saleType == TYPE_AUCTION) {
      signer = _getSigner(_hashAuctionTypedData(_tokenId, _auctioner, _nftAddress), _signature);
    }
