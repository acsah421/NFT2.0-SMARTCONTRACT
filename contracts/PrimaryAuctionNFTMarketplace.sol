...
contract PrimaryAuctionNFTMarketPlace is AccessControlEnumerable, ReentrancyGuard, EIP712 {
  ...
  */
  ...
  function createAuction(Metadata memory _metadata, bytes calldata _signature) external payable nonReentrant {
    ...
    NftInfo memory _nftInfo = NftInfo(_metadata.royaltyReceiver, _metadata.royaltyPercentage, _metadata.IPFSHash);

    _validateMetadata(_metadata.tokenId, _metadata.auctioner, _metadata.nftAddress, _nftInfo, _signature);
    require(_metadata.salePrice > 0, 'Create Auction: Zero sale price.');
    require(_metadata.basePrice > 0, 'Create Auction : Zero base price.');

    placeBid(_metadata.tokenId, _metadata.bidPrice, _metadata.nftAddress, _metadata.auctioner, _nftInfo, _signature);
   ...
   * @param _nftAddress ERC721 or ERC1155 address
   * @param _auctioner Seller address
   * @param _nftInfo Royalty and IPFS details
   * @param _signature Metadata signature signed by admin during NFT creation on platform
   */
  function placeBid(
    uint256 _tokenId,
    ...
    _validateMetadata(_tokenId, _auctioner, _nftAddress, _nftInfo, _signature);
  ...
  function instantBuyNFT(
    ...
    _NftAuction(_tokenId, _nftAddress, _auctioner, buyer, _nftInfo);
  }
  ...
  function instantBuyNFTwithFiat(
    ...
    _NftAuction(_tokenId, _nftAddress, _auctioner, _buyer, _nftInfo);
  }
  ...
  function _NftAuction(
    ...
    Auction storage _nftOnAuction = mapAuction[_nftAddress][_tokenId][_auctioner];
    _mintNFT(_buyer, _nftAddress, _tokenId, _nftOnAuction.quantity, _nftInfo.royaltyReceiver, _nftInfo.royaltyPercentage, _nftInfo.IPFSHash);

    _fundTransfer(_nftOnAuction.salePrice, _nftOnAuction.erc20Token, true, _nftOnAuction.auctioner);

    emit AuctionSettled(
      _nftOnAuction.tokenId,
      _nftAddress,
      _auctioner,
      _buyer,
      _nftOnAuction.erc20Token,
      _nftOnAuction.quantity,
      _nftOnAuction.salePrice
    );
  ...
  function settleAuction(
    ...
    if (NftOnAuction.currentBidder != address(0)) {
      _mintNFT(
        ...
        _nftInfo.royaltyReceiver,
        _nftInfo.royaltyPercentage,
        _nftInfo.IPFSHash
      );
      ...
      _hashTypedDataV4(
        keccak256(
          abi.encode(
            ...
            _nftInfo.royaltyReceiver,
            _nftInfo.royaltyPercentage,
            keccak256(bytes(_nftInfo.IPFSHash))
          )
        )
      );
  }
  ...
  function _verifySignature(
    ...
    address signer = _getSigner(_hashTypedData(tokenId, auctioner, nftAddress, _nftInfo), signature);
  ...
  function _validateMetadata(
    ...
    _verifySignature(_tokenId, _nftAddress, _auctioner, _nftInfo, _signature);
