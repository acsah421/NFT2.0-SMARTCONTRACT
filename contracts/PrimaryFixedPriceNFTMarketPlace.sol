...
contract PrimaryFixedPriceNFTMarketPlace is AccessControlEnumerable, ReentrancyGuard, EIP712 {
  ...
  */
  ...
  function buyNft(
    ...
    require(msg.sender != _metadata.seller, 'BuyNfts : Owner Is Not Allowed To Buy Nfts');
    ...
    _NftSale(_metadata, buyAmount, buyer, _purchasingQuantity);
  }
  ...
  function _NftSale(
    ...
    _mintNFT(
      _buyer,
      _nftOnSale.nftAddress,
      _nftOnSale.tokenId,
      _purchasingQuantity,
      _nftOnSale.royaltyReceiver,
      _nftOnSale.royaltyPercentage,
      _nftOnSale.IPFSHash
    );
    _payOut(_buyAmount, _nftOnSale.erc20Token, _nftOnSale.seller);

    emit NftSold(_nftOnSale.tokenId, _nftOnSale.nftAddress, _nftOnSale.seller, _nftOnSale.price, _nftOnSale.erc20Token, _buyer, _purchasingQuantity);
  }
   ...
   * @param NftForSale NFT metadata
   * @return Hash Hash of metadata
   */
  function _hashTypedData(Metadata memory NftForSale) internal view returns (bytes32) {
    return
      _hashTypedDataV4(
        keccak256(
          abi.encode(
  ...
  function _verifyNFT(Metadata memory metadata, bytes calldata signature) internal view returns (bool) {
    ...
    address signer = _verifyMinter(_hashTypedData(metadata), signature);
